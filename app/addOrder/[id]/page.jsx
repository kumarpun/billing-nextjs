...
export default function AddOrder({ params }) {
    ...
    const options = [
     ...
     { value: 'Sausage fry', label: 'Sausage fry - NRs 240', price: 240 },
       ...
       { value: 'Chessy crunchu chips', label: 'Chessy crunchu chips - NRs 150', price: 150 },
    ...
    //    { value: 'Steam Buff MO:MO', label: 'Steam Buff MO:MO - NRs 220', price: 220 },
       ...
       { value: 'Jackson chicken', label: 'Jackson chicken - NRs 520', price: 520 },
...
{ value: 'Butterscotch Ice Cream', label: 'Butterscotch Ice Cream - NRs 150', price: 150 },
    ...
    { value: 'Rock glass', label: 'Rock glass - NRs 200', price: 200 },
      ...
      const validateForm = () => {
          ...
          const hasMissingOrderType = selectedItems.some(item => !item.order_type);
          if (hasMissingOrderType) {
              newErrors.order_type = "Please select order type for all items";
          }
      ...
      const handleSubmit = async (e) => {
          ...
          try {
              ...
              const orders = selectedItems.map(item => ({
                  ...
                  order_type: item.order_type // Individual order type
              }));
      ...
      const handleItemSelect = (selectedOptions) => {
          // Preserve existing items and their quantities/order types
          const existingItemsMap = new Map();
          selectedItems.forEach(item => {
              existingItemsMap.set(item.value, item);
          });
  
          // Merge existing items with new selections, preserving quantities and order types
          const mergedItems = selectedOptions.map(option => {
              const existingItem = existingItemsMap.get(option.value);
              if (existingItem) {
                  // Return existing item with preserved quantity and order type
                  return existingItem;
              } else {
                  ...
                  return {
                      ...option,
                      quantity: 1,
                      order_type: "" // Initially empty for new items
                  };
              }
          });
      ...
      const updateItemOrderType = (index, orderType) => {
          setSelectedItems(prev => 
              prev.map((item, i) => 
                  i === index ? { ...item, order_type: orderType } : item
              )
          );
  
          // Clear order type error if it exists
          if (errors.order_type) {
              const hasMissingOrderType = selectedItems.some((item, i) => 
                  i !== index ? !item.order_type : false
              );
              if (!hasMissingOrderType) {
                              ...
                              <form onSubmit={handleSubmit} className="space-y-6">
                                  ...
                                  <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                                      ...
                                      <div className="xl:col-span-1 space-y-6">
                                          {/* Item Selection */}
                                          <div>
                                              <label className="block text-gray-300 mb-2 text-sm font-medium">Select Items</label>
                                              <Select 
                                                  isMulti
                                                  options={options}
                                              ...
                                              <div className="bg-gray-700 rounded-lg p-4 border border-gray-600">
                                                  ...
                                                  <div className="border-t border-gray-600 pt-2">
                                                      <div className="flex justify-between items-center">
                                                          ...
                                                          <span className="text-white font-bold text-lg">NRs. {totalPrice}</span>
                                                      </div>
                                                  </div>
                                              </div>
                                          )}
                                                  ...
                                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-h-[500px] overflow-y-auto pr-2">
                                                      {selectedItems.map((item, index) => (
                                                          <div key={index} className="bg-gray-700 rounded-lg p-3 border border-gray-600 hover:border-gray-500 transition-colors">
                                                              ...
                                                              <div className="mb-3">
                                                                  <label className="block text-gray-300 mb-1 text-xs font-medium">Order Type</label>
                                                                  <div className="grid grid-cols-2 gap-2">
                                                                      <button
                                                                          type="button"
                                                                          onClick={() => updateItemOrderType(index, "Kitchen")}
                                                                          className={`py-2 px-2 rounded text-center text-xs font-medium transition-all duration-200 ${
                                                                      ...
                                                                      <button
                                                                          type="button"
                                                                          onClick={() => updateItemOrderType(index, "Bar")}
                                                                          className={`py-2 px-2 rounded text-center text-xs font-medium transition-all duration-200 ${
                                                                              item.order_type === "Bar" 
                                                              ...
                                                              <div className="flex justify-between items-center">
                                                                  ...
                                                                  <div className="text-right">
                                                                      ...
                                                                      <div className={`text-xs ${item.order_type ? "text-green-400" : "text-red-400"}`}>
